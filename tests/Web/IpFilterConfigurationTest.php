...
final class IpFilterConfigurationTest extends TestCase
{
    public function testDefaultDefinitionAllowsLoopbackAndRejectsUnknownClients(): void
    {
        ...
        self::assertSame(
            ...
            'Loopback client must reach the protected toolbar action.',
        );
        self::assertSame(
            403,
            $denied->getStatusCode(),
            'Unknown client must be rejected before the toolbar action.',
        );
        self::assertSame(
            'Forbidden',
            (string) $denied->getBody(),
            'Yii IpFilter must return an explicit rejection.',
        );
    }
    ...
    public function testDefinitionSupportsConfiguredIpRanges(): void
    {
        ...
        self::assertIsArray(
            $debug,
            'Package debug parameters must be present.',
        );

        $debug['allowedIPs'] = ['10.0.0.0/8'];
        $params['yii3/debug'] = $debug;

        $allowed = HelperFactory::createRequest(
            'GET',
            '/debug/toolbar',
            serverParams: ['REMOTE_ADDR' => '10.25.0.8'],
        );
        $denied = HelperFactory::createRequest(
            'GET',
            '/debug/toolbar',
            serverParams: ['REMOTE_ADDR' => '203.0.113.10'],
        );
        ...
        self::assertSame(
            204,
            $this->process($params, $allowed)->getStatusCode(),
            'Configured CIDR must be allowed.',
        );
        self::assertSame(
            403,
            $this->process($params, $denied)->getStatusCode(),
            'Address outside the CIDR must fail.',
        );
    }
    ...
    private function process(array $params, ServerRequestInterface $request): ResponseInterface
    {
        ...
        self::assertInstanceOf(
            RouteGroup::class,
            $group,
            'Debugger routes must share a protected group.',
        );
        ...
        self::assertInstanceOf(
            Closure::class,
            $filterFactory,
            'The route group must expose its IpFilter factory.',
        );

        $filter = $filterFactory(HelperFactory::createResponseFactory(), new Validator());

        self::assertInstanceOf(
            IpFilter::class,
            $filter,
            'Route configuration must build Yii IpFilter.',
        );
