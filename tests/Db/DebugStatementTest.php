...
final class DebugStatementTest extends TestCase
{
    public function testExecutedStatementsReportTheDriverRowCount(): void
    {
        ...
        self::assertInstanceOf(DebugStatement::class, $insert, 'PDO must build the instrumented statement class.');
        $collector->begin('INSERT', 0.0, []);
        self::assertTrue($insert->execute([1, 'alpha']), 'The driver return value must survive.');
        $collector->end('INSERT', 1.0);
        $insert->execute([2, 'beta']);
        $update = $pdo->prepare('UPDATE items SET name = ?');
        self::assertInstanceOf(DebugStatement::class, $update, 'PDO must build the instrumented statement class.');
        $collector->begin('UPDATE', 1.0, []);
        $update->execute(['gamma']);
        $collector->end('UPDATE', 2.0);
        $select = $pdo->prepare('SELECT id FROM items');
        self::assertInstanceOf(DebugStatement::class, $select, 'PDO must build the instrumented statement class.');
        $collector->begin('SELECT', 2.0, []);
        $select->execute();
        $collector->end('SELECT', 3.0);
        self::assertSame([1, 2, 0], $this->reported($collector), 'Counts must be the unmodified driver values.');
    }
    ...
    public function testThrowPDOExceptionWhenExecutionIsRejectedWithoutReportingACount(): void
    {
        ...
        try {
            ...
            self::fail('A primary-key conflict must reach the caller.');
        } catch (PDOException $exception) {
            self::assertStringContainsString('UNIQUE', $exception->getMessage(), 'Driver diagnostics must survive.');
        }

        $collector->end('INSERT', 1.0);
        self::assertSame([null], $this->reported($collector), 'A rejected attempt must overwrite the earlier count.');
    }
