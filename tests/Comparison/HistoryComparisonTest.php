...
final class HistoryComparisonTest extends TestCase
{
    ...
    public function differenceCountSumsAddedRemovedAndChangedCounters(): void
    {
        $original = new HistoryPanelComparison(
            'request',
            'Request',
            new HistoryPanelStates('Captured', 'Captured'),
        ...
        self::assertNotSame(
            $original,
            $panel,
            'Difference configuration must return a new comparison.',
        );
        self::assertNotSame(
            $panel,
            $withStates,
            'State configuration must return a new comparison.',
        );
        self::assertSame(
            0,
            $original->differenceCount(),
            'The original counts must remain unchanged.',
        );
        self::assertSame(
            'Captured',
            $panel->baselineState(),
            'The original panel state must remain unchanged.',
        );
        self::assertSame(
            'Failed',
            $withStates->baselineState(),
            'The configured panel state must be retained.',
        );
    }
    ...
    public function fromSnapshotsFingerprintsFailureChangesWithoutRenderingFailureValues(): void
    {
        ...
        $baseline = new DebugSnapshot(
            ...
            [],
            [
                'request' => PanelFailure::fromThrowable(
                    PanelFailure::CAPTURE,
                    new RuntimeException('baseline secret'),
                ),
            ],
        );
        $target = new DebugSnapshot(
            ...
            [
                'request' => PanelFailure::fromThrowable(
                    PanelFailure::CAPTURE,
                    new RuntimeException('target secret'),
                ),
            ],
        );
    ...
    public function metricConfigurationMethodsReturnCopies(): void
    {
        ...
        $metric = $original->withPanelId('profiling');

        self::assertNotSame(
            $original,
            $metric,
            'Panel configuration must return a new comparison.',
        );
        self::assertNull(
            $original->panelId(),
            'The original panel ID must remain unchanged.',
        );
        self::assertSame(
            '10.00 ms',
            $metric->baseline(),
            'The configured baseline must be retained.',
        );
        self::assertSame(
            '15.00 ms',
            $metric->target(),
            'The configured target must be retained.',
        );
        self::assertSame(
            '+5.00 ms (+50.0%)',
            $metric->delta(),
            'The configured delta must be retained.',
        );
        self::assertSame(
            'up',
            $metric->trend(),
            'The configured trend must be retained.',
        );
        self::assertSame(
            'profiling',
            $metric->panelId(),
