...
final class DbPanelTest extends TestCase
{
    ...
    public function testCountChipTitleReportsExecutedQueriesOrActiveWarnings(
        ...
        self::assertSame(
            $expected,
            $items[0]->title ?? null,
            'Tooltip must match the active threshold state.'
        );
        self::assertCount(
            1,
            $items,
            'Database must expose only the query-count chip for every threshold state.'
        );
    }
    ...
    public function testExplainLinksUseConfiguredPrefixAndStoredSequenceOnly(): void
    {
        ...
        self::assertStringContainsString(
            ...
            $html,
            'Plans must use the configured prefix.'
        );
        self::assertStringContainsString(
            'Explain all',
            $html,
            'Supported drivers must expose the shared batch control.'
        );
        self::assertStringNotContainsString(
            'sql=',
            $html,
            'Plan URLs must never carry client-supplied SQL.'
        );
    }
    ...
    public function testFiltersPreserveNavigationAndExposeRecoveryWhenNoRowsMatch(): void
    {
        ...
        self::assertStringContainsString(
            'name="Db[type]"',
            $html,
            'SQL verbs must use the shared select control.',
        );
        self::assertStringContainsString(
            'name="Db[query]"',
            $html,
            'SQL text must use the shared text control.',
        );
        self::assertStringContainsString(
            'alpha',
            $html,
            'Matching SQL must remain visible.',
        );
        self::assertStringNotContainsString(
            'UPDATE beta',
            strip_tags($html),
            'Combined filters must exclude other rows.',
        );
        self::assertStringContainsString(
            'yii_debug_theme=dark',
            $html,
            'Filter and sort links must preserve the theme.',
        );
        self::assertStringNotContainsString(
            'unknown',
            $html,
            'Unknown filter keys must not survive normalized URLs.',
        );
        self::assertStringNotContainsString(
            'page=999',
            $html,
        ...
        self::assertStringContainsString(
            ...
            $html,
            'No-match states must differ from empty captures.',
        );
        self::assertStringContainsString(
            'Clear all',
            $html,
    ...
    public function testMetadataEmptyCaptureAndContextFreeGrid(): void
    {
        ...
        self::assertSame(
            'db',
            $panel->id(),
            'Database must use its canonical panel ID.',
        );
        self::assertSame(
            'Database',
            $panel->name(),
            'The shared panel title must be preserved.',
        );
        self::assertSame(
            'db',
            $panel->icon(),
            'The shared Database icon must be reused.',
        );
        self::assertFalse(
            $panel->hasContent([]),
            'Missing captures must not be advertised.',
        );
        self::assertTrue(
            $panel->hasContent(['entries' => []]),
            'Empty valid captures must remain inspectable.',
        );
        self::assertSame(
            [],
            $panel->toolbarItems(['entries' => []]),
            'Empty Database toolbars must omit query metrics.',
        );
        self::assertStringContainsString(
        ...
        self::assertStringContainsString(
            ...
            $html,
            'Database must use the shared grid CSS.',
        );
        self::assertStringContainsString(
            '<strong>3</strong> queries',
            $html,
        ...
        foreach ($headings as $heading) {
            self::assertStringContainsString(
                $heading,
                $html,
                'The grid must retain every Yii2 Database column.',
            );
        }
        ...
        self::assertSame(
            '3',
            $items[0]->value ?? null,
            'Toolbar count must include all queries.',
        );
        self::assertCount(
            1,
            $items,
            'The toolbar must omit the database duration chip.',
        );
        self::assertStringContainsString(
            '<strong>6.000</strong> ms total',
            $html,
            'Query duration must remain in the Database panel.',
        );
        self::assertSame(
            'info',
            $items[0]->status,
            'Disabled thresholds must not warn.',
        );
        self::assertSame(
            'warning',
            $panel
                ->withThresholds(2, null)
                ->toolbarItems(DatabaseFixture::snapshot()->jsonSerialize())[0]->status ?? null,
            'Critical query counts must warn.',
        );
        self::assertSame(
            'info',
            $panel
                ->withThresholds(3, null)
                ->toolbarItems(DatabaseFixture::snapshot()->jsonSerialize())[0]->status ?? null,
            'The query threshold must be exclusive.',
        );
        self::assertSame(
            'warning',
            $panel
                ->withThresholds(null, 3)
                ->toolbarItems(DatabaseFixture::snapshot()->jsonSerialize())[0]->status ?? null,
            'The caller threshold must be inclusive.',
        );
    }
    ...
    public function testNPlusOneMarkersArePageScopedAndSqlAndTracesAreEscaped(): void
    {
        ...
        self::assertStringContainsString(
            ...
            $html,
            'N+1 summaries must state their scope.',
        );
        self::assertStringContainsString(
            'data-yii-debug-n1-group',
            $html,
            'Matching rows must expose existing JS markers.'
        );
        self::assertStringNotContainsString(
            '<script>',
            $html,
            'Captured SQL and traces must not become executable markup.'
        );
        self::assertStringNotContainsString(
    ...
    public function testThresholdCopiesPreserveAlertsAndPanelDuration(): void
    {
        $panel = new DbPanel(new DbExplain(), new DebugUrlGenerator(), Trace::create());
        ...
        self::assertNotSame(
            $panel,
            $copy,
            'Threshold configuration must return a copy.'
        );
        ...
        self::assertCount(
            ...
            'Even long-running queries must have no toolbar duration chip.'
        );
        self::assertStringContainsString(
        ...
        self::assertSame(
            'info',
            $panel->toolbarItems($payload)[0]->status,
            'The original panel must keep disabled thresholds.'
        );
    }
    ...
    public function testVisibleColumnsSortBeforePagination(string $sort, string $first): void
    {
        ...
        self::assertSame(
            ...
            'The sorted first query must be the only visible row.'
        );
        self::assertStringContainsString(
            'aria-sort=',
            $html,
            'The active column must announce its sort direction.'
        );
    }
