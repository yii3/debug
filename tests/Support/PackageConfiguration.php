...
final class PackageConfiguration
{
    ...
    /**
     * Returns the converter `config/di/collectors.php` hands to the Mail collector.
     *
     * @throws RuntimeException when the packaged definition carries no converter.
     *
     * @return Closure(MessageInterface): Email Converter building the Symfony email the adapter sends.
     */
    public static function emailFactory(): Closure
    {
        $definition = self::serviceDefinitions('collectors', self::params())[MailCollector::class] ?? null;

        $arguments = is_array($definition) ? $definition['__construct()'] ?? null : null;
        $factory = is_array($arguments) ? $arguments['emailFactory'] ?? null : null;

        if (!$factory instanceof Closure) {
            throw new RuntimeException('The packaged Mail collector definition must carry an email converter.');
        }

        return static function (MessageInterface $message) use ($factory): Email {
            $email = $factory($message);

            if (!$email instanceof Email) {
                throw new RuntimeException('The packaged email converter must return a Symfony email.');
            }
    ...
    /**
     * Loads one `config/di/*.php` file with the params in scope.
     *
     * @param string $name File name without the `.php` extension.
     * @param array<string, mixed> $params Complete params array.
     *
     * @throws RuntimeException when the packaged file does not return a string-keyed array.
     *
     * @return array<string, mixed> Definitions the file declares.
     */
    public static function serviceDefinitions(string $name, array $params): array
    {
        $file = dirname(__DIR__, 2) . "/config/di/{$name}.php";
